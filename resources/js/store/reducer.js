...
const reducer = (state = initialState, action) => {
    ...
    switch (action.type) {
        case actionTypes.SET_DEFAULT:
            const {application_forms_id} = localStorage.getItem('userData')? JSON.parse(localStorage.getItem('userData')).user : 'Null';  
            let default_Path = state.defaultPath;
            if(application_forms_id==0){
                
                default_Path = '/application-form';
            }
        return {
                ...state,
        ...
        case actionTypes.USER_LOGIN:
            const {parmissions} = localStorage.getItem('userData')? JSON.parse(localStorage.getItem('userData')).user : 'Null';
            return {
